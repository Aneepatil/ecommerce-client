...
import { createProductionAction } from "../../../redux/slice/products/productSlice";
...
export default function AddProduct() {
  ...
  //  Select category data from store
  ...
  const colorOptionsCoverted = color?.map((color) => {
    return {
      value: color?.name,
      label: color?.name,
    };
  ...
  const handleOnSubmit = (e) => {
    ...
    dispatch(
      createProductionAction({
        ...
        sizes: sizeOption?.map((size) => size),
      })
    );
  ...
  return (
    <>
      ...
      {fileError?.length>0 && <ErrorMsg message={'File is too large or upload an image'} />}
